...
class BuildPerksTile extends Component {
  constructor(props) {
    super(props);
    this.state = {isOpenModal:false,pendingModalAccepted:false, isApplePerkInfoModalOpen: false};
  }
  ...
  render() {
    ...
    const {perkInfo={},isOpenModal,pendingModalAccepted, isApplePerkInfoModalOpen} = this.state;
    ...
    return (
      <BuildPerkTile>
      <BuildPerkTile data-testid="buildPerksTile" tabIndex="0" aria-Label={perkInfo?.PerkDetails?.description ? perkInfo.PerkDetails.description:perkInfo?.perkTitle ? perkInfo.perkTitle:"Perk"} role="heading" aria-level={"2"} data-perkid={perkId} data-perktoggle={togglePerk ? "true":"false"}onClick= {(e)=>{
        toggleViewDetails();
...
e.stopPropagation()
                ...
                <Toggle
                  ...
                  onChange={(e)=>{                    
                    ShowPerkTile ? this.RemoveMutualExclusionsPerks(perkInfo?.PerkDetails?.radioButtonData):this.toggleSelect(perkId)}}
                  on={togglePerk}
